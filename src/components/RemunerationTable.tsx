...
interface NonRemunerativeItem extends SalaryItem {
    isAttendanceRow?: boolean;
    isSeniorityRow?: boolean;
    referenceItemId?: number;
}

interface DeductionItem extends SalaryItem {
    ...
    const handleNonRemunerativeAmountChange = (id: number, value: string) => {
        ...
        const updatedItems = tempRows.map((row) => {
            if (row.referenceItemId === id && row.isAttendanceRow) {
                ...
                const seniorityAmount =
                    seniorityRow?.amount
                        ? Number.parseFloat(
                            seniorityRow.amount.replace(/\./g, "").replace(",", ".")
                        ) || 0
                        : 0;
                ...
                const percentage =
                    Number.parseFloat(row.percentage.replace(/\./g, "").replace(",", ".")) /
                    100;
    
                console.log({ baseAmount, seniorityAmount, total, percentage });
                ...
                <CardContent>
                    ...
                    <div className="mt-8 p-6 bg-primary/10 rounded-lg border-2 border-primary">
                        <div className="flex justify-between items-center text-xl font-bold">
                            <span>
                                Total Sueldo Bruto (Remunerativo + No
                                Remunerativo)
                            </span>
                            <span>
                                {formatNumber(
